...
module ActiveShipping
  class UPS < Carrier
    ...
    def get_delivery_dates(origin, destination, packages, pickup_date=Date.current, options = {})
      origin, destination = upsified_location(origin), upsified_location(destination)
      options = @options.merge(options)
      packages = Array(packages)
      access_request = build_access_request
    ...
    def parse_delivery_dates_response(origin, destination, packages, response, options={})
      ...
      if success
        xml.css('ServiceSummary').each do |service_summary|
          # vvvvv Translate the Time in Transit Codes to the service codes used elsewhere vvvvv
      ...
      response = DeliveryDatesResponse.new(success, message, Hash.from_xml(response).values.first, :delivery_estimates => delivery_estimates, :xml => response, :request => last_request)
    end
